#!/usr/bin/env node
/**
 * 帮帮林 · 考研·就业全周期规划 — 双端口后端服务器（零依赖纯 Node）
 *
 * 端口：
 *   3210  用户端   - 任何人注册/登录就可用；index.html 由服务器直接托管
 *   3211  管理端   - 2815097621@qq.com 登录可用；admin.html 独立管理面板
 *                    管理员可以：查看所有用户、对照官网、增改真题/岗位、联网刷新
 *                    也可用 admin 账号登录 3210 作为普通用户使用
 *
 * 数据共享：users.json / anon.json / exams.json / jobs.json
 *
 * 启动：
 *   node server.js
 *   或 PORT_USER=3210 PORT_ADMIN=3211 node server.js
 *
 * 联网配置（可选）：
 *   EXAMS_SOURCE = https://example.com/exams.json   // 外部真题源（JSON 数组）
 *   JOBS_SOURCE  = https://example.com/jobs.json    // 外部岗位源（JSON 数组）
 *   OFFICIAL_PORTAL = https://yz.chsi.com.cn        // 官方网站参照（仅显示）
 */
'use strict';
...
function touch(p, def) { if (!fs.existsSync(p)) fs.writeFileSync(p, JSON.stringify(def, null, 2)); }
...
function refreshExternal() {
  ...
  if (EXAMS_SOURCE) {
    tasks.push(fetchJson(EXAMS_SOURCE).then(function (arr) {
      ...
      var ex = readExams();
      var ids = {}; ex.items.forEach(function (it) { ids[it.id] = true; });
      ...
      ex.lastUpdated = now; writeExams(ex); result.exams = true;
    ...
    ex2.lastUpdated = now; writeExams(ex2);
    result.exams = true; result.note = '演示同步（未配置外部真题源）';
  }
  if (JOBS_SOURCE) {
    tasks.push(fetchJson(JOBS_SOURCE).then(function (arr) {
      if (!Array.isArray(arr) || !arr.length) return;
      var jb = readJobs();
      var ids = {}; jb.items.forEach(function (it) { ids[it.id] = true; });
      arr.forEach(function (it) {
        if (it && it.id && !ids[it.id]) {
          it.addedAt = now; it.pulledAt = now;
          it.sourceUrl = it.sourceUrl || it.url || JOBS_SOURCE;
          jb.items.unshift(it); ids[it.id] = true;
        }
      });
      jb.lastUpdated = now; writeJobs(jb); result.jobs = true;
    ...
    jb2.lastUpdated = now; writeJobs(jb2);
    result.jobs = true;
  }
...
function handleApi(req, res, parsed) {
  ...
  if (pathname === '/api/admin/exams' && method === 'POST') {
    parseBody(req).then(function (body) {
      ...
      var exi = readExams(); exi.items.unshift(item); exi.lastUpdated = item.addedAt; writeExams(exi);
      json(res, 200, { ok: true });
    });
    return;
  }
  ...
  if (pathname === '/api/admin/jobs' && method === 'POST') {
    parseBody(req).then(function (body) {
      ...
      var jbi = readJobs(); jbi.items.unshift(item); jbi.lastUpdated = item.addedAt; writeJobs(jbi);
      json(res, 200, { ok: true });
    });
    return;
  }
...
(function seedAdmin() {
  try {
    ...
    if (!store.users[ADMIN_EMAIL]) {
      ...
      store.users[ADMIN_EMAIL].isAdmin = true;
      writeUsers(store);
